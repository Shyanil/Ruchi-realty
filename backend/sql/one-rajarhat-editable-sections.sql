...
SELECT p.title, sp.hero_title, 
       jsonb_array_length(sp.specifications) AS specifications_count,
       jsonb_array_length(sp.gallery_images) AS gallery_images_count,
       sp.hero_bg, sp.is_published, sp.updated_at
FROM public.project_subpages sp
JOIN public.projects p ON p.id = sp.project_id
WHERE p.title ILIKE '%One Rajarhat%';
