...
function HeroSection({ subpage, onBrochureClick }) {
  return (
    <header className="osc-hero one-prime-hero" data-screen-label="One Prime Residential">
      <div className="osc-hero__bg"><picture><source media="(max-width: 640px)" srcSet={subpage.heroMobileUrl || fallbackData.heroMobileUrl} /><img src={subpage.heroBg || fallbackData.heroBg} alt={subpage.heroTitle} /></picture></div>
      <div className="osc-hero__overlay"></div><div className="osc-hero__sig" aria-hidden="true"></div>
      <div className="rr-wrap osc-hero__wrap"><Reveal><div className="osc-hero__content">
        {subpage.heroLogo ? <img className="osc-hero__logo one-prime-hero__logo" src={subpage.heroLogo} alt="One Prime Residential logo" loading="eager" /> : null}
        <h1 className="osc-hero__title">{subpage.heroTitle}</h1><p className="osc-hero__city">Kolkata</p><p className="osc-hero__tagline">{subpage.heroTagline}</p>
        <div className="osc-hero__actions"><button className="submit-btn" onClick={() => { window.location.href = "/projects"; }}>More Projects<CtaArrow /></button><button className="ab-btn-outline ab-btn-outline--white" onClick={onBrochureClick}>Download Brochure<CtaArrow /></button></div>
      </div></Reveal></div>
      <div className="osc-hero__chips">{(subpage.overviewHighlights || []).map((h, i) => <Reveal key={h.label || i} delay={i * 80} className="osc-chip"><HighlightIcon icon={h.icon} /><span className="osc-chip__label">{h.label}</span><span className="osc-chip__desc">{h.desc}</span></Reveal>)}</div>
    </header>
  );
}
