...
async function callMsg91(action, mobile, otp) {
  const url = new URL(action === "verify" ? `${MSG91_BASE_URL}/verify` : action === "resend" ? `${MSG91_BASE_URL}/retry` : MSG91_BASE_URL);
  url.searchParams.set("mobile", mobile); url.searchParams.set("authkey", MSG91_AUTH_KEY);
  const request = { headers: { Accept: "application/json", authkey: MSG91_AUTH_KEY } };
  if (action === "send") { url.searchParams.set("template_id", MSG91_TEMPLATE_ID); request.method = "POST"; request.headers["Content-Type"] = "application/json"; request.body = "{}"; }
