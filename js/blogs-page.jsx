/* ============================================================
   Blog page — public insights catalogue + reader
   ============================================================ */
const { useEffect: uEB, useMemo: uMB, useState: uSB } = React;
...
function BlogPage() {
  const [items, setItems] = uSB(() => BLOG.map(cleanBlog));
  const [activeCategory, setActiveCategory] = uSB("All");
  const [activeSlug, setActiveSlug] = uSB(() => location.hash.replace(/^#/, ""));
  ...
  const categories = uMB(() => ["All", ...Array.from(new Set(items.map((item) => item.cat))).filter(Boolean)], [items]);
  ...
  uEB(() => {
    if (!selected) return;
    const t = setTimeout(() => smoothTo("#article"), 80);
  ...
  const openArticle = (slug) => {
    ...
    setActiveSlug(slug);
    setTimeout(() => smoothTo("#article"), 40);
  };

  const closeArticle = () => {
    history.pushState(null, "", location.pathname);
    setActiveSlug("");
    setTimeout(() => smoothTo("#insights"), 40);
  };
