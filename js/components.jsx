/* ============================================================
   Ruchi Realty — Shared components (v2)
   ============================================================ */
...
function RImg({ src, alt = "", className = "", style = {}, grade = false, mono = true }) {
...
function AwardSeal({ id, year }) {
  return (
    <svg viewBox="0 0 100 100" fill="none" aria-hidden="true" className="seal">
      <defs>
        <linearGradient id={`sl-${id}`} x1="82%" y1="6%" x2="14%" y2="94%">
      ...
      {/* outer hairline ring */}
      <circle cx="50" cy="50" r="44" stroke={`url(#sl-${id})`} strokeWidth="1.25" />
      {/* inner ring with small notches */}
      <circle cx="50" cy="50" r="36" stroke={`url(#sl-${id})`} strokeWidth="0.75" strokeOpacity="0.55" strokeDasharray="1.5 4.5" />
      {/* center star */}
      <path d="M50 33 l3.6 7.4 8.1 1.1 -5.9 5.7 1.4 8.1 -7.2 -3.8 -7.2 3.8 1.4 -8.1 -5.9 -5.7 8.1 -1.1z"
        fill={`url(#sl-${id})`} fillOpacity="0.9" />
      {/* twin laurel ticks at base */}
      <path d="M34 70 q6 6 16 6 q10 0 16 -6" stroke={`url(#sl-${id})`} strokeWidth="1" strokeLinecap="round" fill="none" />
    </svg>
  );
}
...
Object.assign(window, { Reveal, RImg, StatCounter, AwardSeal, Icon, CustomCursor });
